<template>
  <el-row class="list-container">
    ...
    <el-col>
      <el-form :inline="true" :model="areaForm" label-width="80px">
        ...
        <el-form-item label="区/县">
          <el-input v-model="areaForm.countyName" clearable></el-input>
        </el-form-item>
    ...
    <el-col :offset="19" :span="5">
      <el-button v-if="buttonPermission.indexOf('countyList') > -1" type="primary" icon="el-icon-search" @click="loadTable">查询</el-button>
      <el-button v-if="buttonPermission.indexOf('exportCounty') > -1" type="primary" icon="el-icon-document" @click="fileExport" >导出</el-button>
    </el-col>
    <el-col :offset="1" :span="23" class="margin-top20">
      <!--      <el-button icon="el-icon-edit-outline" @click="createCounty" round>新建</el-button>-->
      <!--      <el-button icon="el-icon-edit-outline" @click="updateCounty" round>修改</el-button>-->
      <el-button v-if="buttonPermission.indexOf('deleteCounty') > -1" type="primary" icon="el-icon-edit-outline" @click="deleteCounty" :disabled="isBtnDelete" round>删除</el-button>
    </el-col>
  ...
  export default {
    ...
    data() {
      return {
        ...
        sonChecked: [],
        exportData:[],
        roleOptions: ['国际代码', '省份名称', '城市名称', '区/县名称', '区/县拼音', '状态', '修改时间', '修改人'], //传显示导出字段的数据到子窗口
        tableData: [],
        multipleSelection: [],
        page: {
          total: 10,
    ...
    methods: {
        ...
        if (checked.length > 0){
          for(let i = 0; i < checked.length; i++){
            switch(checked[i]){
              ...
              case "区/县名称": array[i] = "countyName";break;
              case "区/县拼音": array[i] = "countyPhonetic";break;
      ...
      confirm() {
        this.$axios.get('/api/orgmanagement/area/county/getCountyList',
          {params:
              {
      ...
      updateProvince() {
        if (this.checkChoiceNum()) {
          //跳转
          this.$router.push('/province/detail')
        }
      },
      //  删除
      deleteCounty() {
        if (this.checkChoiceNum()) {
          this.$confirm('确定删除省份？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            this.$axios.get('/api/orgmanagement/area/county/deleteCountyById',
              {
                params:
                  {
