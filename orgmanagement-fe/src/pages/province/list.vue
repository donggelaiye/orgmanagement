<template>
  <el-row class="list-container">
    ...
    <el-col :offset="19" :span="5">
      <el-button v-if="buttonPermission.indexOf('provinceList') > -1" type="primary" icon="el-icon-search" @click="loadTable">查询</el-button>
      <el-button v-if="buttonPermission.indexOf('exportProvince') > -1" type="primary" icon="el-icon-document" @click="fileExport" >导出</el-button>
    </el-col>
    <el-col :offset="1" :span="23" class="margin-top20">
      <!--      <el-button icon="el-icon-edit-outline" @click="createProvince" round>新建</el-button>-->
      <!--      <el-button icon="el-icon-edit-outline" @click="updateProvince" round>修改</el-button>-->
      <el-button v-if="buttonPermission.indexOf('deleteProvince') > -1" type="primary" icon="el-icon-edit-outline" @click="deleteProvince" :disabled="isBtnDelete" round>删除</el-button>
    </el-col>
  ...
  export default {
    ...
    data() {
      return {
        ...
        sonChecked: [],
        exportData:[],
        roleOptions: ['国际代码', '省份名称', '省份拼音', '状态', '修改时间', '修改人'], //传显示导出字段的数据到子窗口
        tableData: [],
        multipleSelection: [],
    ...
    methods: {
        ...
        if (checked.length > 0){
          for(let i = 0; i < checked.length; i++){
            switch(checked[i]){
              ...
              case "省份拼音": array[i] = "provincePhonetic";break;
      ...
      confirm() {
        this.$axios.get('/api/orgmanagement/area/province/getProvinceList',
          {params:
              {
      ...
      deleteProvince() {
        if (this.checkChoiceNum()) {
          this.$confirm('确定删除省份？', '提示', {
            ...
            this.$axios.get('/api/orgmanagement/area/province/deleteProvinceById',
              {
                params:
                  {
